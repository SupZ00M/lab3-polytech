     ...
     */
{
    ...
    {
        WorkerDb db;
        db["Ivanov"] = WorkerData("Ivan", 34, "",1);
        db["Petrov"] = WorkerData("Petr", 43, "",1);
     ...
     */
{
    ...
    Notification n1 = Notification::make_system (10, "Battery low",   Severity::NORMAL);
    Notification n2 = Notification::make_message(20, "Alice", "Hi!");
    Notification n3 = Notification::make_app    (30, "Telegram", "New post", "Hello world");
    Notification n4 = Notification::make_system (40, "Fire alarm!",  Severity::URGENT);

    print_notification(n1);
    print_notification(n2);
    print_notification(n3);
    print_notification(n4);

    Notification arr[4] = { n1, n2, n3, n4 };
    std::cout << "MESSAGE count: " << count_by_type(arr, 4, NotificationType::MESSAGE) << "\n";
    std::cout << "SYSTEM  count: " << count_by_type(arr, 4, NotificationType::SYSTEM)  << "\n";
    std::cout << "APP     count: " << count_by_type(arr, 4, NotificationType::APP)     << "\n";

   
    NotificationQueue q;
    q.push(n1); q.push(n2); q.push(n3);
    std::cout << "q.size() = " << q.size() << "\n";
    Notification a = q.pop();   // n1
    Notification b = q.pop();   // n2
    std::cout << "FIFO pop: ";
    a.print(); b.print();
    std::cout << "q.size() = " << q.size() << "\n";
    ...
    std::cout << "Priority pop order:\n";
    while (pq.size() > 0) {
        Notification x = pq.pop();
        x.print();
    }
}
    /**
...

   {
    ...
    {
        BaseFile f1(const_cast<char*>("test.txt"), const_cast<char*>("w"));
        // f1 владеет FILE*

        BaseFile f2(std::move(f1));
        // f2 "забрал" FILE* у f1.
        // f1.file == nullptr, f1 деструктор не закроет файл.

        std::cout << "f1.is_open() = " << f1.is_open() << "\n";  // 0
        std::cout << "f2.is_open() = " << f2.is_open() << "\n";  // 1

        f2.write("hello", 5);
    }   // здесь разрушается только f2 — файл закрывается один раз.

    std::cout << "\n=== Move-оператор присваивания ===\n";
    {
        BaseFile a(const_cast<char*>("a.txt"), const_cast<char*>("w"));
        BaseFile b(const_cast<char*>("b.txt"), const_cast<char*>("w"));

        std::cout << "before: a.is_open()=" << a.is_open()
                  << ", b.is_open()=" << b.is_open() << "\n";

        b = std::move(a);
        // b закрывает свой старый FILE* (b.txt),
        // потом "забирает" FILE* у a.
        // a.file == nullptr.

        std::cout << "after:  a.is_open()=" << a.is_open()
                  << ", b.is_open()=" << b.is_open() << "\n";

        b.write("data", 4);
    }   // разрушается b — файл закрывается один раз.

    std::cout << "\n=== Попытка копирования — не компилируется ===\n";
    // BaseFile x("x.txt", "w");
    // BaseFile y = x;             // ← ОШИБКА КОМПИЛЯЦИИ
    // BaseFile z; z = x;          // ← ОШИБКА КОМПИЛЯЦИИ
